...
case class ConfigWrapper(conf: Config) {
  ...
  def getOrElse[T](path: String, fallback: T): T =
    try {
      fallback match {
      ...
      case _: ConfigException.Missing =>
        println(
          s"Missing configuration value at $path using fallback value $fallback")
        fallback
      case _: ConfigException.WrongType =>
        println(
          s"Wrong configuration value type at $path using fallback value $fallback")
        fallback
    }
